...
백준링크 : https://www.acmicpc.net/problem/3135
...
const curr = data[0]; // 주파수1
const target = data[1]; // 주파수2
const favorArr = []; //즐겨찾기 주파수 배열

const inArr = []; // 현재주파수와 목표 주파수 사이에 있는 즐겨찾기 주파수

for (let i = 2; i < input.length; i++) {
  favorArr.push(input[i]);
}
...
for (let i = 0; i < favorArr.length; i++) {
  let abs = Math.abs(target - favorArr[i]);
  let result = favorArr[i] === curr ? abs : abs + 1;
  subArr.push(result);
}
